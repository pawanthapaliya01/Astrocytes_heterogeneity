...
plot(time1,data,'o','MarkerSize',5','MarkerEdgeColor','r','LineWidth', 2,...
    'DisplayName','EXP')
%plot(time1,dataInmda,'s','MarkerSize',5','MarkerEdgeColor','r','LineWidth', 3)
%xlabel('time s')
%ylabel('INMDA pa')
hold on
plot(t,y1,'-b','LineWidth', 2,'DisplayName','SIM')
%plot(t,Inmda,'-b','Linewidth',2)
xlabel('Time(s)')
ylabel('Open Probability')
legend('FontName','Times Roman New','FontSize',12,'FontWeight','bold','location','northeast')
legend 'boxoff'
ytickformat('%.1f')
xtickformat('%.1f')
ax=gca;
